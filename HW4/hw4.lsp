...
(defun node2var (n c k)
  (+ c (* k (- n 1))) ;;; follows the convention that variable index = (n − 1) * k + c
)
...
(defun at-most-one-color (n c k)
  (cond ((>= c k) nil)
        (t (append (helper n c (+ c 1) k) (at-most-one-color n (+ c 1) k))))
)

(defun helper (n c cn k)
  (cond ((> cn k) nil)
        (t (cons (append (list (- (node2var n c k)))(list (- (node2var n cn k)))) (helper n c (+ cn 1) k))))
)
...
(defun edgehelper (e c k)
  (cond ((> c k) nil)
        (t (cons (append (list (- (node2var (car e) c k)))(list (- (node2var (car (cdr e)) c k))))(edgehelper e (+ c 1) k))))
)

; I discussed this assignment with Chang Xie, but I wrote the codes by myself.
